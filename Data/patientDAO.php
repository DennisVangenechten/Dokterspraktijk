...
class PatientDAO
{
    ...
    public function checkRrForUpdate($id, $rr)
    {
        try {
            // Open database connection
            $dbh = new PDO(
            ...
            // SQL Statement to check if the rr already exists (excluding the current patient)
            $sql = "SELECT rr FROM patienten WHERE rr = :rr AND id != :id";
            $stmt = $dbh->prepare($sql);
            $stmt->bindValue(':rr', $rr, PDO::PARAM_STR);
            ...
            if ($result) {
                return "Another patient with the same rijksregisternummer already exists.";
            } else {
                return false;
            }
