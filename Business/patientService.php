...
use Data\PatientDAO;
use Exceptions\PatientAlreadyExistsException;
...
class PatientService
{
    ...
    public function newPatient(): void
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $existingPatientMessage = $dao->checkRr($rr);
            if ($existingPatientMessage) {
                throw new PatientAlreadyExistsException($existingPatientMessage);
            }
    ...
    public function updatePatient()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            // Check for duplicate rr (excluding the current patient)
            $existingPatientMessage = $dao->checkRrForUpdate($id, $rr);
            if ($existingPatientMessage) {
                throw new PatientAlreadyExistsException($existingPatientMessage);
            }
            ...
            if ($id && $rr && $name && $surname && $gender && $dateOfBirth && $street && $number && $city && $zipcode && $email && $phone) {
                ...
                $checkCity = $dao->checkCity($city, $zipcode);
                if ($checkCity) {
                    $result = $dao->update(...array_values($newPatientData));
                    if ($result) {
                        echo "Patient updated successfully!";
                    } else {
                        echo "Failed to update patient. Please try again.";
                    }
                }
            } else {
                echo "Please fill in all required fields correctly.";
